...
export function LampContainer({ children, className }) {
  return (
    <div
      className={cn(
        "relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-slate-950 w-full rounded-md z-0 max-w-[100vw]",
        className
      )}
    >
      <div className="absolute inset-0 flex items-center justify-center isolate z-0 w-full">
        <div className="relative flex w-full flex-none items-center justify-center isolate z-0 -translate-y-[12rem] sm:-translate-y-[20rem]">
          {/* RIGHT GLOW */}
          <motion.div
      ...
      <div className="relative z-50 flex w-full flex-col items-center justify-center px-5">
        {children}
      </div>
    </div>
  );
